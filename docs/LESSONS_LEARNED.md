...
| LL-T04 | Vérifier par commande exécutable ou recherche dédiée toute précondition factuelle avant de l'écrire dans une analyse/PDR — y compris un mécanisme de plateforme externe ou du contenu fourni comme acquis | 9 | SDLC-14 (origine), SDLC-16, SDLC-22, SDLC-23, SDLC-25, ECO-1, ECO-2, SDLC-Audit-Strands-Harness, SDLC-Import-Strands-Harness | **Gradué `/retrospective` 03/09/2026** | Promu en règle permanente — `Claude.md §Analyse` (nouveau bloc "Vérification factuelle") + `01-Claude-md-TEMPLATE.md` (même bloc, générique). *Correction de comptage : SDLC-25 et ECO-1 avaient été classées "nouveau" à l'exécution — ce sont en réalité des confirmations de ce pattern, reclassées ici sans réécrire leurs entrées sources.* |
