...
## §Index des patterns · mis à jour 03/09/2026 · Sprints SDLC-07→ECO-4
...
| LL-T08 | `sprint-memory.md` réduit à son en-tête pour un sprint au diff substantiel, malgré des décisions réelles arbitrées en conversation et jamais tracées en DÉCISION/TEST | 3 | SDLC-25, ECO-1, ECO-4 | Actif — seuil de surveillance atteint | 3e occurrence — le déclencheur que l'entrée elle-même nommait est atteint. Décision de garde-fou (ex : seuil `git diff --stat` vs nombre d'entrées `sprint-memory.md`) différée à la prochaine `/retrospective`, pas tranchée ici (hors format Lessons Learned). |
