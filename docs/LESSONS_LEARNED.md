...
## §Index des patterns · mis à jour 24/09/2026 · Sprints SDLC-07→ECO-8
...
| LL-T04 | Vérifier par commande exécutable ou recherche dédiée toute précondition factuelle avant de l'écrire dans une analyse/PDR — y compris un mécanisme de plateforme externe ou du contenu fourni comme acquis | 10 | SDLC-14 (origine), SDLC-16, SDLC-22, SDLC-23, SDLC-25, ECO-1, ECO-2, SDLC-Audit-Strands-Harness, SDLC-Import-Strands-Harness, ECO-7 | **Gradué `/retrospective` 03/09/2026** | Promu en règle permanente — `Claude.md §Analyse` (nouveau bloc "Vérification factuelle") + `01-Claude-md-TEMPLATE.md` (même bloc, générique). *Correction de comptage : SDLC-25 et ECO-1 avaient été classées "nouveau" à l'exécution — ce sont en réalité des confirmations de ce pattern, reclassées ici sans réécrire leurs entrées sources.* |
...
| LL-T12 | Un critère d'acceptation utilisant `grep -A<N>` dimensionne `N` sur une estimation plutôt que le contenu réel du fichier cible — la commande de vérification échoue alors qu'un contenu correct existe | 3 | ECO-3 (`-A40`→`-A50`), ECO-4 (`-A10`→`-A20`, `-A3`→`-A12`, 2 critères), ECO-8 (grep préalable du §Handoff avec `$` interprété en regex — même famille : commande de vérification écrite sans être exécutée, P-25 l'aurait attrapée) | Nouveau — corrigé | Rappel ajouté à `04-sprint-PDR-TEMPLATE.md §Critères d'acceptation` (`/retrospective` SDLC-27, 03/09/2026) — compter les lignes réellement couvertes avant d'écrire `-A<N>` |
...
| LL-T14 | Un total, comptage ou identifiant écrit de mémoire dans un livrable/commentaire/décision (pas dans une analyse) dérive du réel, distinct de `LL-T04` par le moment (rédaction, pas analyse) | 4 | SDLC-28 (`LL-T13`), SDLC-29 (nom de fichier planifié non confronté au disque), SDLC-30 (4 comptages faux : blocs de commandes, templates, version en commentaire, nombre de sprints), ECO-7 (PDR : 3 contrôles sans incident au lieu de 4, citation « fencée » inexistante) | **Gradué `/retrospective` 22/09/2026** | `M-PROC-48` — extension de `Claude.md §Analyse §Vérification factuelle` (+ `01-Claude-md-TEMPLATE.md` v2.6) : tout total écrit dans un livrable vérifié par commande, préférer `N/M` à un nombre figé quand il peut changer |
...
### Sprint ECO-8 — 24/09/2026 — Suite de tests du validateur au wrap-up + C8 sur `tests/`
**Code :** C8 couvre `tests/*.sh` (cas RED vu FAIL avant, PASS après) · suite 16/16 · validateur 10/10. Ce wrap-up est le premier à lancer la suite via l'Étape 3.5.
**Processus :** PDR rédigé en session Claude Code (pas de PDR Claude.ai), à partir des points (a) et (b) du `SDLC_CANDIDATE` d'ECO-7 — clos ; le point (c) reste ouvert. Le grep préalable du §Handoff (`grep -n 'for f in "$ROOT"'`) ne renvoyait rien : écrit sans être lancé.
**Lien pattern :** confirme `LL-T12` (3ᵉ occurrence, forme élargie : commande de vérification d'un PDR jamais exécutée avant d'être écrite)

### Sprint ECO-7 — 23/09/2026 — Durcissement de `sdlc-validate.sh` (harnais de test, C3 au grain ligne)
**Code :** `tests/sdlc-validate-test.sh` 15/15 · `sdlc-validate.sh` 10/10. Les 2 cas de non-aveuglement C3 ont échoué avant le durcissement (résidu en prose dans `06-PDR-bootstrap.md`/`07-DECISIONS-SDLC.md` → exit 0) : la cécité de `C3_EXCEPTIONS` est prouvée, pas seulement supposée. Fixture sans `exemples/` (91 Mo) : suite en ~0,3 s.
**Processus :** sprint né d'une passe de lecture externe (passe A) dont la prémisse était fausse — elle amendait ECO-1, livré 3 semaines plus tôt ; contre-vérifiée en repo avant tout PDR. Le PDR corrigé portait encore 2 affirmations non mesurées (« C2, C6, C8 » sans incident — 4 en réalité ; marqueurs « fencés à l'Étape 2 » — forme échappée, non matchée), trouvées en mesurant avant de coder. Critère `bash -n a b` défectueux (ne vérifie que `a`).
**Lien pattern :** confirme `LL-T04` (prémisse de la passe A non vérifiée contre l'état du repo) · confirme `LL-T14` (comptages écrits de mémoire dans le PDR)
**SDLC candidat :** [SDLC_CANDIDATE] *(→ Mise à jour 24/09/2026 : (a) et (b) implémentés — Sprint ECO-8 ; (c) ouvert)* (a) C8 ne couvre pas `tests/*.sh` (glob racine + `.claude/hooks/`) ; (b) l'Étape 3.5 du wrap-up et la checklist `00-CONTEXT.md §4` ne lancent pas `bash tests/sdlc-validate-test.sh` ; (c) règle de la passe A — avant toute lecture de source externe destinée au modèle, établir l'état courant du repo (`git log`, scripts existants lancés) et le citer au §Contexte, rattachée à `LL-T04` (2ᵉ occurrence après la sur-transposition d'`E-15`) → fichier cible : `sdlc-validate.sh` C8 · `03-wrap-up-SKILL-TEMPLATE.md §3.5` + skill vivant · `00-CONTEXT.md §4` · `12-audit-externe-TEMPLATE.md` · nature : contrôle étendu / procédure modifiée — décision : [en attente — à remonter manuellement dans projet SDLC]

### Sprint SDLC-Import-Strands-Harness — 23/09/2026 — Import Strands Harness R3 + R4 (P-45)
**Code :** N/A — templates et 1 commentaire de hook. `bash sdlc-validate.sh` → 10/10, exit 0. Erreur de fait trouvée en construisant le tableau R4 : « `exit 1` = bloquer (silencieux) » dans `08-hooks-TEMPLATE.md` et `.claude/hooks/pre-tool-bash.sh:9` depuis au moins SDLC-18 — la doc Claude Code dit l'inverse (erreur non bloquante, seul `exit 2` bloque). Sans effet réel (aucun `exit 1` actif), mais diffusé aux projets cibles comme un moyen de bloquer.
**Processus :** 2ᵉ PDR Claude.ai consécutif sur `doc/` au lieu de `docs/` — cette fois en demandant de *propager* la convention périmée (« corriger `docs/` → `doc/` », critère à 0 occurrence). Revu avant 4a, 9 corrections appliquées sur aval. Ma propre revue du PDR a affirmé un échec C3 sans lire `C3_EXCEPTIONS` — rétracté en 4d.
**Lien pattern :** confirme `LL-T04` (2 occurrences : PDR Claude.ai + revue Claude Code) · proche de `LL-T13` (affirmation fausse persistante faute de contrôle) mais sur un fait de plateforme, pas un identifiant — non comptée comme 2ᵉ occurrence.
**SDLC candidat :** [SDLC_CANDIDATE] rappel des chemins réels (`docs/`, `07-DECISIONS-SDLC.md`, dernier `M-XXXX-NN`) pour les PDR rédigés dans Claude.ai — 2 sprints consécutifs → fichier cible : `04-sprint-PDR-TEMPLATE.md` et/ou instructions du projet Claude.ai · nature : règle renforcée — décision : [en attente — à remonter manuellement dans projet SDLC]
**SDLC candidat :** [SDLC_CANDIDATE] toute affirmation sur le comportement de Claude Code dans un template (codes de sortie, schémas JSON) porte une source datée, comme le schéma `PreCompact` (`M-HOOKS-08`) → fichier cible : `08-hooks-TEMPLATE.md` / `01-Claude-md-TEMPLATE.md §Vérification factuelle` · nature : règle renforcée — décision : [en attente — à remonter manuellement dans projet SDLC]
