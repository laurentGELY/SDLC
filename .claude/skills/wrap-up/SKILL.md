# wrap-up — SKILL
<!-- Adapté depuis 03-wrap-up-SKILL-TEMPLATE.md (SDLC v1.6) · Sprint SDLC-14 (self-bootstrap) · GSD-V1+V2 appliqués Sprint SDLC-GSD-V2 -->
<!-- Projet toolkit SDLC — pas de code applicatif, gouvernance de templates -->
...
ni présumer du contexte déjà discuté. Chercher : incohérences, doublons,
contenu fictif présenté comme un fait, lacunes visibles dans le diff seul.
...
Relecture du même diff avec accès complet au projet (templates, fichiers
existants, conventions `STANDARDS.md`). Chercher : incohérences avec les
fichiers existants, divergence avec les conventions établies (numérotation,
structure), références cassées vers d'autres fichiers.

**Couche 3 — Acceptance Auditor (conditionnel — fichier registre touché)**
...
Couche 1 : [N findings]
Couche 2 : [N findings]
Couche 3 : [N findings / N/A — fichier registre non touché]
...
**Lien pattern :** [confirme LL-TXX / infirme LL-TXX / nouveau LL-TXX / aucun]
...
- **`fichier.md` vX.Y** : [description modification]
- **Tests** : [résultat]
```
...
Lire `07-DECISIONS-SDLC.md`. Identifier les entrées existantes liées au travail de ce sprint.
...
gouvernance : les fichiers projet Claude.ai sont **hors repo git** —
...
```
Convention du modèle modifiée    → CHANGELOG + SPEC.md
Tuning d'une règle existante     → CHANGELOG + 07-DECISIONS-SDLC.md (avant/après)
Fichier registre ±               → STANDARDS §Modules partagés (même commit)
Spike                            → 07-DECISIONS-SDLC.md uniquement (pas de template)
Sprint Doc                       → CHANGELOG uniquement
Incident / rattrapage            → LESSONS_LEARNED §Action proposée prioritaire
