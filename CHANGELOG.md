...
- **`01-Claude-md-TEMPLATE.md` v2.3** : `§Analyse §Auto-revue du plan` (nouveau, entre `§Vérification factuelle` et `§Demande d'aval`) — 3 passes (couverture spec / placeholders / cohérence des références) + règle d'arrêt (`E-14`) ; format `DÉCISION` de `§Mémoire de sprint` +1 champ `[coût si faux]` (`E-06`) ; +1 règle de précédence « ledger + `git log` > souvenir de session » après compaction (`E-07`)
- **`07-DECISIONS-SDLC.md`** : entrée `M-PROC-43` (5 champs), tableau de compatibilité
- **Correction factuelle avant rédaction du PDR (`M-PROC-41` appliquée nativement)** : `E-07` de l'ANALYSE affirmait la ligne d'identité du ledger manquante dans le modèle — infirmé pour moitié, elle existe déjà (`§Démarrage 4b` + clause d'exception). Portée réduite à la seule règle de précédence, absente elle.
- **Hors scope, explicitement** : `E-13(a)` (Interfaces Consumes/Produces) et `E-13(c)` (dimensionnement des tâches) — non demandés par `docs/ROADMAP.md §Next` ; `08-hooks-TEMPLATE.md` — condition de `E-06` vérifiée non applicable (`PreCompact` n'écrit jamais de ligne `DÉCISION`) ; `Claude.md` (ce repo) — précédent SDLC-19, pas de nouvelle règle absolue opérative dans ce sprint
- **`.claude/hooks/pre-tool-bash.sh`** (hors PDR initial, ajouté sur aval explicite pendant le wrap-up) : carve-out `M-HOOKS-04` élargi à `.claude/sprint-memory.md` lui-même (`|| "$FILE_PATH" == */.claude/sprint-memory.md`) — clôt `docs/LESSONS_LEARNED.md LL-T07`, dont le déclencheur de réouverture nommait explicitement ce sprint (`/retrospective` 03/09/2026), manqué à la rédaction du PDR (parti de l'ANALYSE/ROADMAP, pas de LESSONS_LEARNED) et repéré seulement en préparant l'Étape 2 du wrap-up. 2 smoke tests positif/négatif exécutés (`07-DECISIONS-SDLC.md M-HOOKS-04 → Mise à jour 03/09/2026`)
- **Corrections ajustées vs spec** — deux critères d'acceptation de `specs/Sprints/sprint-ECO-4-durcissement-pdr.md` avaient une fenêtre `grep -A` trop courte pour le contenu réel (`-A10`→`-A20` pour `§Pas de placeholders`, `-A3`→`-A12` pour `M-PROC-43`) — même défaut que le wrap-up `ECO-3`, contenu déjà correct, commandes corrigées avant commit ; ajout hors §Portée du carve-out `pre-tool-bash.sh` (voir ci-dessus), traité sur aval explicite plutôt que différé
- **Tests** : niveau A — `bash sdlc-validate.sh` → **8/8 ✅**, exit 0 ; smoke tests carve-out `pre-tool-bash.sh` — positif/négatif OK
...
- **Substitution `\bdoc/` → `docs/`** sur ~30 fichiers versionnés actifs (templates `0X`/`1X`, `Claude.md`, `STANDARDS.md`, skills installés, scripts `sdlc-*.sh`, `.gitignore`, `00-CONTEXT.md`, `specs/SPEC.md`, `07-DECISIONS-SDLC.md`) — `specs/Sprints/*.md` et `bak/sdlc-kit-exportable.md` exclus (récits historiques, même principe que `CHANGELOG.md`)
- **`00-CONTEXT.md` v1.5** : nouvelle section « Site de documentation publique » dans §1 · **`specs/SPEC.md`** : ligne site dans §Modules
- **`07-DECISIONS-SDLC.md`** : entrée `M-ARCH-09` (convention `doc/` → `docs/`)
- **Renumérotation** : ce sprint était initialement nommé "SDLC-24" dans le PDR reçu — collision avec un sprint déjà clos (2026-06-22, fix schéma JSON `08-hooks-TEMPLATE.md`) → renuméroté SDLC-25 (validé utilisateur)
- **Publication GitHub Pages** : activation "Deploy from branch → main → /docs" — action humaine hors Claude Code, non exécutée dans ce sprint
- **`sdlc-init.sh`** (fix, `M-ARCH-05` mis à jour) : (1) bug préexistant sans rapport avec ce sprint — `sed` de `deploy_template()` cassé par un délimiteur `/` en collision avec le format de date `JJ/MM/AAAA` en remplacement, script interrompu dès `Claude.md` → **`sdlc-init.sh` n'avait jamais pu bootstrapper un projet avec succès**, corrigé (délimiteur `#`) ; (2) régression de ce sprint — `mkdir -p .../doc` (guillemet, pas de `/`, invisible au grep de non-régression) laissait un dossier `doc/` vide dans chaque projet bootstrappé, corrigé (`docs/`). Découverts en exécutant réellement le test niveau B (jamais fait en conditions réelles jusqu'ici), corrigés et revalidés (exit 0, structure complète, zéro dossier résiduel) avant ce commit.
- **Tests** : niveau A — 12 critères d'acceptation vérifiés par commande (grep de non-régression `\bdoc/` vide, `bash -n` sur les 3 scripts, `M-ARCH-09` présente, historique git préservé) ✅ · niveau B — `bash sdlc-init.sh` exécuté dans un repo git temporaire isolé (`/tmp/eco-init-test-sdlc25-v2`), exit 0, structure complète, zéro dossier `doc/` résiduel ✅ (2 défauts trouvés et corrigés au passage, voir bullet `sdlc-init.sh` ci-dessus)
- **Corrections ajustées vs spec** — Adversarial Review au wrap-up (Taille M) a trouvé 6 défauts non détectés en session d'exécution (falsification historique `README.md`, compte erroné `M-ARCH-09`, numérotation `nav.json`, version/historique du site, libellé `/sdlc-init`), tous corrigés avant commit, **plus** les 2 défauts de `sdlc-init.sh` trouvés par le test niveau B lui-même. Détail complet : `specs/Sprints/sprint-SDLC-25-docs-migration-site.md §Corrections ajustées vs spec`.
